// Biometric Authentication — AIB Platform
...
const TOKEN_KEY = '@aib_biometric_token';
const BIOMETRIC_ENABLED_KEY = '@aib_biometric_enabled';
...
export async function authenticateWithBiometric(reason?: string): Promise<boolean> {
  ...
  const result = await LocalAuthentication.authenticateAsync({
    promptMessage: reason ?? 'Authenticate to access AIB Platform',
    cancelLabel: 'Cancel',
...
export async function saveTokenBiometric(token: string): Promise<void> {
  await SecureStore.setItemAsync(TOKEN_KEY, token, {
    ...
    authenticationPrompt: 'Authenticate to save credentials for AIB Platform',
  });
...
export async function getTokenBiometric(): Promise<string | null> {
  ...
  try {
    return await SecureStore.getItemAsync(TOKEN_KEY, {
      requireAuthentication: true,
      authenticationPrompt: 'Authenticate to sign in to AIB Platform',
    });
  } catch {
    return null;
  }
